...
export const addWallet = async (profileCode: string, wallet: Wallet): Promise<Wallet> => {
  console.debug('Adding wallet:', wallet);
  const saved = await apiClient.postJson<Wallet>(basePath(profileCode), wallet);
  return saved;
};

export const updateWallet = async (profileCode: string, wallet: Wallet): Promise<Wallet> => {
  console.debug('Updating wallet:', wallet);
  const saved = await apiClient.putJson<Wallet>(`${basePath(profileCode)}/${wallet.code}`, wallet);
  return saved;
};

export const deleteWallet = async (profileCode: string, wallet: Wallet): Promise<void> => {
