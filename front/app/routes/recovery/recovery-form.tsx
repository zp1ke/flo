...
export default function RecoveryForm({ code }: { code: string }) {
  ...
  const onRecover = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      await recoverPassword({ code, email, password });
      setSaved(true);
    } catch (e) {
  ...
  if (saved) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl">{t('recovery.saved')}</CardTitle>
          <CardDescription>{t('recovery.savedDescription')}</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <div className="flex flex-col gap-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl">{t('recovery.title')}</CardTitle>
            ...
            <form onSubmit={form.handleSubmit(onRecover)} className="space-y-8">
              <div className="flex flex-col gap-6">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="grid gap-2">
                      <FormLabel>{t('recovery.email')}</FormLabel>
                      <FormControl>
                        <Input
                  ...
                  render={({ field }) => (
                    <FormItem className="grid gap-2">
                      <div className="flex items-center">
                        <FormLabel>{t('recovery.password')}</FormLabel>
                      </div>
                      <FormControl>
                        <Input
                          ...
                          required
                          disabled={processing}
                          autoComplete="recovery-password"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
                ...
                <Button type="submit" className="w-full" disabled={processing}>
                  ...
                  {processing && t('recovery.processing')}
                  {!processing && t('recovery.action')}
                </Button>
              </div>
            </form>
          </Form>
